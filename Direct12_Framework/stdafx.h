...
bool FindChunk(std::ifstream& hFile, DWORD fourcc, DWORD& dwChunkSize, DWORD& dwChunkDataPosition);
bool ReadChunkData(std::ifstream& hFile, void* buffer, DWORD buffersize, DWORD bufferoffset);
AudioData LoadWavFile(const std::string& filename, WAVEFORMATEXTENSIBLE& waveFormat);
...
namespace FileLoad
{
	inline int ReadStringFromFile(std::ifstream& iFile, std::string& str)
	{
		int strLength = 0;
		iFile.read((char*)&strLength, sizeof(int));
		str.resize(strLength);
		iFile.read(&str[0], strLength);
		return strLength;
	}

	template <class T>
	inline T ReadFromFile(std::ifstream& iFile)
	{
		T rvalue;
		iFile.read((char*)&rvalue, sizeof(T));
		return rvalue;
	}

	template <class T>
	inline void ReadFromFile(std::ifstream& iFile, T* buffer, const int& count)
	{
		iFile.read((char*)buffer, sizeof(T) * count);
	}
