...
{
	UINT input_element_desc_count = 2;
...
{
	return(CompileShaderFromFile(L"StaticMeshShader.hlsl", "VSDiffused", "vs_5_1", shader_blob));
}

D3D12_SHADER_BYTECODE StaticMeshShader::CreatePixelShader(ID3DBlob** shader_blob)
{
	return(CompileShaderFromFile(L"StaticMeshShader.hlsl", "PSDiffused", "ps_5_1", shader_blob));
}
